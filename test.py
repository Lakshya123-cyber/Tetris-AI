...
WIDTH_COEFF = (
    3  # For regressive count of width on certain height (more for less effect)
)
...
def get_piece(field):
    ...
    for i in range(10):
        ...
        if (
            leftover_flag
            and i < 9
            and (field[i + 1, top] != 0 or field[i + 1, top + 1] != 0)
            and (field[i, top] == 0 and field[i, top + 1] == 0)
        ):
...
def do_permutations(piece, floor):
    ...
    print(
        "Best pos:", best_posiion, "Best rot:", best_rotation, "Best score:", best_score
    )
